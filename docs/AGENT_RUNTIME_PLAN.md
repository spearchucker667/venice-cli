...
* **SYSTEM**: Agent behavior contract
* **PROJECT**: Repository instructions and metadata
* **WORKING MEMORY**: Current objective, Todo state, Decisions, Known failures, Relevant files
* **CONVERSATION**: Recent user/assistant messages
* **FILE CONTEXT**: Only currently relevant file content
* **TOOL RESULTS**: Recent/high-value results
* **SUMMARY**: Structured compacted history
...
`git diff` or equivalent structured diff output.
...
* **First Ctrl+C**: cancel current model/tool operation
* **Second Ctrl+C**: exit session
...
* **macOS/Linux**: user shell or explicit `/bin/sh` strategy
* **Windows**: PowerShell or existing process execution strategy
* **WSL**: normal Linux behavior
...
* **Tool Registry**: registration, duplicate names, schema validation, tool lookup, tool failures, source metadata.
* **Workspace**: Git root detection, non-Git directories, symlinks, nested repositories, external paths.
* **Permissions**: read, write, execute, network, destructive operations, session-specific grants.
* **Sessions**: create, persist, resume, corrupt data, concurrent access, deletion, export.
* **Context**: budget estimation, compaction trigger, important-state retention, oversized tool output.
* **Shell**: success, failure, timeout, stderr, cancellation, working directory.
* **File Editing**: normal edits, stale content, UTF-8, CRLF, missing file, write failure.
* **Instructions**: global, repository, nested, precedence, missing files.
* **MCP**: process startup, discovery, tool conversion, execution, process crash, timeout, malformed response.
...
1. User asks to inspect `package.json`.
2. Model requests `read_file`.
3. Tool returns `package.json`.
4. Model requests shell `npm test`.
5. Permission is approved.
6. Shell returns failure.
7. Model reads failing test file.
8. Model applies patch.
9. Model reruns test.
10. Model returns final answer.
